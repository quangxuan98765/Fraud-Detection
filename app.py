...
def get_fraud_stats():
    try:
        with detector.driver.session() as session:
            # Thống kê giao dịch đơn giản hóa
...
def get_metrics():
    try:
        with detector.driver.session() as session:            # Truy vấn tối ưu để lấy thống kê
            basic_metrics = session.run("""
                ...
                WITH total_accounts, count(r) AS total_transactions
                  // Đếm tài khoản gian lận với ngưỡng phù hợp với cấu hình
                MATCH (f:Account)
                WHERE f.fraud_score > $fraud_threshold
                WITH total_accounts, total_transactions, count(f) AS detected_fraud_accounts
                  // Đếm giao dịch được phát hiện là gian lận - sử dụng ngưỡng thấp hơn
            ...
            """, fraud_threshold=FRAUD_SCORE_THRESHOLD).single()              # Phân phối điểm gian lận - giúp hiểu rõ hơn về phân bố
            ...
            """, 
                ...
                suspicious_threshold=SUSPICIOUS_THRESHOLD).single()              # Thống kê về cộng đồng
            communities = session.run("""
                MATCH (a:Account)
            ...
            if basic_metrics:
                ...
                # Xử lý trường hợp không có giao dịch nào được phát hiện hoặc không có giao dịch gian lận thực sự
                if detected == 0 and actual == 0:
                    ...
                    precision = 0
                    recall = 100.0  # Recall = 100% khi không có gian lận thực sự nào để phát hiện
                else:
                    # Precision = TP / (TP + FP) 
...
def get_suspicious_accounts():
    try:
        with detector.driver.session() as session:            # Truy vấn cải tiến để lấy các tài khoản đáng ngờ có thêm thông tin chi tiết
            suspicious = session.run("""
                MATCH (a:Account)
                WHERE a.fraud_score > $suspicious_threshold  // Sử dụng ngưỡng từ cấu hình
...
def get_network():
    try:
        with detector.driver.session() as session:
            ...
            print(f"Total relationships in database: {rel_count['count']}")              # Sử dụng truy vấn mới để lấy mạng lưới gian lận với ngưỡng cấu hình
            ...
            if top_nodes_result and "relationships" in top_nodes_result:
                ...
                for rel in relationships:
                    ...
                    is_suspicious = 1 if (start_score > FRAUD_SCORE_THRESHOLD or end_score > FRAUD_SCORE_THRESHOLD) else 0
